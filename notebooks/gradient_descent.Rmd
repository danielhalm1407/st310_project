...
### Introducing the context for logistic gradient descent
...
```{r}
# need to set a seed otherwise the "best" 3-predictor model changes every run from the randomness in the `initial_split` function
set.seed(6849)

Heart_split <- initial_split(Heart, prop = 0.7)

subset_selection <- regsubsets(target ~ ., data = Heart_split %>% training())
summary(subset_selection)

predictor_names <- c("exang", "oldpeak", "ca")

training <- Heart_split %>% training() %>% select(all_of(predictor_names), "target")
testing <- Heart_split %>% testing() %>% select(all_of(predictor_names), "target")
```

We see that the best 3-predictor model includes `exang`, `oldpeak`, and `ca` which are exercised induced angina, ST depression induced by angina, and the number of major vessels colored by fluoroscopy. We analyze the correlations between these predictors and see that there are some moderately positive correlation happening as shown in the correlation heatmap below. This correlation suggests some multicollinearity is happening.
...
### Explanation of why feature scaling was necessary
...
### Simulating training and testing data for a naive model
