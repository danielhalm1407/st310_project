...
```{r}
Heart_split <- initial_split(Heart, prop = 0.6)
...
predict(ranger_fit, Heart_valid, type = "prob") %>%
  bind_cols(predict(ranger_fit, Heart_valid)) %>%
  bind_cols(select(Heart_valid, y)) %>%
  metrics(y, .pred_1, estimate = .pred_class)

```

#### Do the Validation itself
```{r}
...
# Create a grid for mtry (1 to 6)
mtry_grid <- tibble(mtry = rep(1:6,2), trees = c(rep(100,6),rep(200,6)))

# Tune mtry using grid search
