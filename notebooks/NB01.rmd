...
# most of the below are needed due to the process behind generating partial dependence plots (PDPs)
...
```
```{r, echo=F}
# check we have extracted the right model
...
```{r, echo = F}
# check that we have selected the right model
best_params <- tree_output$accuracy_df[best_spec_idx,]
best_params %>% glimpse()
```
#### Check the Accuracy of the Best Model 
...
```{r, echo = F}
# CHECK extraction of the right model
best_params <- tree_output$accuracy_df[best_spec_idx,]
best_params %>% glimpse()
```
#### Check the Accuracy of the Best Model 
...
```
```{r, echo = F}
# inspect the tuning grid set-up
tuning_grid_rf
```
...
```{r, echo = F}
# inspect the accuracy dataframe
...
```{r, echo = F}
# check we have the right dataframe
tuning_grid_rf_step
```
