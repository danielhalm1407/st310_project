...
In the case of heart disease diagnosis, we might not simply want the highest accuracy, but perhaps particularly avoid false negatives. 

On the one hand, false positives may cause undue stress, when someone is misleadingly worried about heart disease that they may not have. However, in general it will tend to have positive effects anyway, since most prescriptions to combat the disease do not have negative outcomes and come at large personal benefits relative to any costs. Eating less sugar and saturated fat leads to a slight quality of life reduction in the short term, but a healthier lifesile has substatial benefits and often improves quality of life in the long term and that derived from profound and not superficial pleasures. However, to the extent that prescriptions are more severe in their nature, such as stents (medicine) or operations, we must approach this with great care,

On the other hand, it is likely worth trading off more false negatives in exchange for more true positives so that a greater amount of corrective treatment can be prescribed, and so that we can prevent the conditions of more patients getting worse.

Likewise, any non-professional (perhaps a patient themselves) could also use such a flow chart structure to identify their own chance of a positive diagnosis/ what factors tend to be most associated with high risk of heart disease.
...
- **Low variance**, because it averages out the predictions of a large number of trees, each fitted on a bootrapped (i.e., subsample with replacement) subsample of the training dataset.

- **Low variance**, because (unlike **bagging**) it allows splits only on a certain subset of predictors, meaning that each tree is less correlated with the next.

These combine to create a model that has **high predictive accuracy** on **new** data, and, ideally, not just on new data from our dataset (i.e., can be used to generalise in the distribution), but also from new datasets (which may be out of distribution, to the extent that the underlying relationships have changed). 

In other words, such a fitting procedure should capture as accurately the **fundamental** or **structural** relationships between the variables that do not change throughout time, as opposed to the **transitory** or **reduced form** relationships. 

Using a validation set approach, we can only guarantee the former, but argue that the fact that the human body works much in the same way today than it did many decades ago means that much of the relationships we capture hold true throughout time.
...
Thus, the fact that there is no-need to sacrifice false positives for diagnostic accuracy means that this model class provides a highly desireable trade-off that is genuine.
