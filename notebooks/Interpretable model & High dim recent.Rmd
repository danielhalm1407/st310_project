...
barplot(coef_plot[, 1], main = "Coefficients of Logistic Model", las = 2)

# Most significant qualitative variable
sort(summary(glm_categoricals)$coefficients[, 4]) # most significant is cp
```

Thus, our interpretable logistic model is built utilising oldpeak, the numerical variable with the highest correlation to the target, and cp, the most significant cateogrical predictor. Thus, our interpretable logit model is:
...
# Plot misclassification rate vs lambda parameter (after finding best lambda)
plot(lasso_model, main = "Misclassification Rate vs Lambda", xlab = "Lambda" , ylab = "Misclassification Rate")
abline(v = log(best_lambda), lty = 2) # Add vertical line for best lambda
...
The model with the pair that yields the highest accuracy rate (ca and oldpeak) is trained. 
...
Causality can be understood via a DAG diagram as illustrated below. 

Our DAG includes predictors which we believe are the six most important towards potentially having heart disease.

In order, with the predictor relatively causing heart disease the most: 
1. oldpeak - this tells us about the short term downward slope that can be seen on an electrocardiograph caused by exercise vs rest.
2. ca - indicative of how many major vessels are potentially some % blocked and their respective sites.
3. trestbps - the higher the blood pressure is at rest, the more likely one is to have heart disease, as well as stroke, heartattack and other chronic heart problems
4. restecg - if the resting electrocardiographic results are abnormal, this shows that there is extra load on the heart and might have valvular defects or cardiovascular problem
5. chol 
6. fbs 

Cholestrol and fasting blood sugar levels, which are towards the bottom of the list, are in that position as they still do cause heart disease but can be controlled via proper attention to diet, medicine and exercise. The other variables require surgical intervention for improvement as are defects which are usually too fargone.

These predictors individually increase the chances of having heart disease, but are also interlinked.

Higher sugar levels give way to higher cholestrol levels. This is because an increase in sugar causes a reduction in HDL (good cholestrol) and an increase in LDL (bad cholestrol). An increase in an individual's cholestrol levels causes an icnrease in plaque build up in arteries as excess cholestrol deposits against artery walls. This narrows the arteries meaning the heart has to work even harder to pump blood around the body. This in turn causes an increase in blood pressure. This increase in blood pressure, also known as hypertension, can be seen on an ECG when exercising, as demonstrated by a downward slope, also known as ST depression. A ST depression is a potential cause of an electrocardiogram test. As a result of the electrocardiogram test, a fluroscopy may be done to confirm any findings. 
...
I decide to only include fbs against target, as fbs is the only predictor which is not confounded in nature.

We utilised tidy models for this, extracting the coefficients for fbs.

The estimate of -0.329 suggests that for a one-unit increase in fasting blood sugar, the log-odds of heart disease decrease by 0.329, assuming no confounding. This is counterintuitive as an increase in fbs should reflect an increased risk of heart disease.

This is because fbs also impacts target via chol, so our naive logit excludes indirect impacts and lacks adjustments for confounders. 

```{r}
# naive logit assuming no confounding
logit_spec <- logistic_reg() %>% 
  set_engine("glm") %>% 
  set_mode("classification")

# Only included fbs as only predictor in DAG with direct relations - rest are confounded
logit_fit <- logit_spec %>% 
  fit(factor(target) ~ fbs, data = train_data)

fbs_coef <- tidy(logit_fit) %>% 
  filter(term == "fbs") 

print(fbs_coef)
```

Thus we add controls to take into account causal relationships. (probably will add chol here)
